 ...
 * deterministic.
 */
export function validateReleaseNotes(changelog, version) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!headingMatch || !versionRef.test(line))
            continue;
        ...
        for (let j = i + 1; j < lines.length; j++) {
            ...
            if (bodyHeadingMatch && bodyHeadingMatch[1].length <= headingLevel)
                break; // next entry begins
            if (body.trim().length > 0)
                return true; // non-empty content found
        }
    }
    return false;
}
 ...
 */
export function boundScorecard(scorecard) {
    return {
...
function healthDelta(current, previous) {
 ...
 */
export function computeHealthMetrics(current, previous) {
    return {
...
export function buildReleaseReport(input) {
    ...
    const validation = { semverOk, releaseNotesOk };
    if (!semverOk) {
