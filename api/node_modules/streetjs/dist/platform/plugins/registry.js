 ...
 */
export function resolveContained(destDir, entryName) {
...
export class PluginInstaller {
    ...
    constructor(opts) {
        ...
        if (this.allowUnsigned) {
            ...
            console.warn('[PluginInstaller] allowUnsigned=true: signature verification is DISABLED. ' +
    ...
    async install(name, version) {
        ...
        if (!this.allowUnsigned) {
            if (!this.trustedKey) {
                throw new Error(`No trusted public key configured for ${name}@${version}; ` +
    ...
    async _fetchManifest(name, version) {
        ...
        if (!result.success) {
            throw new Error(`Manifest for ${name}@${version} failed schema validation: ${result.error.message}`);
        }
        const manifest = result.data;
        if (!manifest.signature) {
            throw new Error(`Manifest for ${name}@${version} is missing a signature. ` +
                `Plugin installation aborted.`);
        }
        return manifest;
    }
    ...
    assertHttps(rawUrl) {
        ...
        if (u.protocol !== 'https:') {
            throw new Error(`Refusing non-https plugin transport: ${u.protocol}// (https required)`);
        }
        return u;
    }
    _fetchText(url) {
    ...
    async _extractTarball(buffer, destDir) {
        ...
        for (const entry of entries) {
            ...
            if (entry.typeFlag === '1' || entry.typeFlag === '2') {
                throw new Error(`Refusing link entry "${entry.name}" (type ${entry.typeFlag}) in plugin archive`);
            }
            // Reject any entry that escapes containment (Req 2.1, 2.2, 2.4).
            const safe = resolveContained(destDir, entry.name);
            if (safe === null) {
                throw new Error(`Refusing path-traversal entry "${entry.name}" outside plugin directory`);
            }
            safePaths.push(safe);
        }
