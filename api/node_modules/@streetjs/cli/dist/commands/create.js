...
export class CreateCommand {
    async execute(ctx) {
        ...
        const starterFlags = new Set();
        if (ctx.args.flags['with-billing'])
            starterFlags.add('with-billing');
        if (ctx.args.flags['with-admin-ui'])
            starterFlags.add('with-admin-ui');
        if (ctx.args.flags['with-email'])
            starterFlags.add('with-email');
        if (ctx.args.flags['with-marzpay'])
            starterFlags.add('with-marzpay');
    ...
    async scaffoldProject(targetDir, projectName, database = 'sqlite') {
        ...
        // street.config.ts
        await writeFile(join(targetDir, 'street.config.ts'), this.renderStreetConfig(projectName, database), 'utf8');
        // src/main.ts
        await writeFile(join(targetDir, 'src/main.ts'), this.renderMainTs(database), 'utf8');
        ...
        await writeFile(join(targetDir, 'src/repositories/example.repository.ts'), this.renderExampleRepository(database), 'utf8');
        ...
        // docker-compose.yml
        await writeFile(join(targetDir, 'docker-compose.yml'), this.renderDockerCompose(database), 'utf8');
        ...
        // .env.example
        await writeFile(join(targetDir, '.env.example'), this.renderEnvExample(database), 'utf8');
        // .gitignore
     ...
     */
    async applyTemplate(targetDir, template, enabledFlags = new Set()) {
        const spec = TEMPLATES[template];
        if (!spec || template === 'app')
            return;
        // Always-on deps + opt-in deps for each enabled flag.
        let deps = { ...spec.packages };
        ...
        for (const file of spec.extraFiles ?? []) {
            if (file.flag && !enabledFlags.has(file.flag))
                continue;
     ...
     */
    async scaffoldFrontend(targetDir, frontend, projectName) {
...
`;
     ...
     */
    async scaffoldHtmxMarzPay(targetDir) {
...
`;
    ...
    async scaffoldCI(targetDir, frontend) {
    ...
    renderWebReactPackageJson(projectName) {
...
`;
    ...
    renderWebIndexHtml(projectName) {
        return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
`;
     ...
     * (Requirement 8.5).
     */
    async scaffoldReactMarzPay(webDir) {
...
`;
    ...
    renderWebNextPackageJson(projectName) {
        return JSON.stringify({
            name: `${projectName}-web`,
            version: '0.1.0',
            private: true,
...
`;
     ...
     */
    async scaffoldNextMarzPay(webDir) {
        ...
        await writeFile(join(webDir, 'app', 'billing', 'success', 'page.tsx'), this.renderNextBillingSuccessPage(), 'utf8');
        await writeFile(join(webDir, 'app', 'billing', 'cancel', 'page.tsx'), this.renderNextBillingCancelPage(), 'utf8');
        await writeFile(join(webDir, 'app', 'api', 'webhooks', 'marzpay', 'route.ts'), this.renderNextMarzPayWebhookRoute(), 'utf8');
    }
    renderNextMarzPayLib() {
...
${webJob}`;
    ...
    renderStreetConfig(_projectName, database = 'sqlite') {
        if (database === 'sqlite') {
...
`;
    ...
    renderExampleRepository(database = 'sqlite') {
        const isSqlite = database === 'sqlite';
...
`;
    ...
    renderDockerCompose(database = 'sqlite') {
        if (database === 'sqlite') {
...
`;
    ...
    renderEnvExample(database = 'sqlite') {
        if (database === 'sqlite') {
