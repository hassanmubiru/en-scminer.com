...
export class UpgradeCommand {
    async execute(ctx) {
        ...
        if (ctx.args.flags['list']) {
            console.log('\n  Available codemods:\n');
            for (const c of core.listCodemods())
                console.log(`  ${c.id.padEnd(30)} ${c.description}`);
            console.log('');
            return;
        }
        ...
        for (const file of files) {
            ...
            try {
                ...
                console.error(`[street] ${err.message}`);
                process.exitCode = 1;
                return;
            }
            if (result.changed) {
                ...
                for (const [id, n] of Object.entries(result.perCodemod)) {
                    if (n > 0)
                        perCodemod[id] = (perCodemod[id] ?? 0) + n;
                }
        ...
        for (const [id, n] of Object.entries(perCodemod))
            console.log(`    - ${id}: ${n}`);
        console.log('');
    }
    /**
     ...
     */
    async reportBreakingChanges(ctx, core) {
        ...
        if (installed === null && toArg === undefined)
            return false;
        let resolution;
        try {
     ...
     */
    async detectInstalledVersion(cwd) {
        return ((await this.readPackageVersion(resolve(cwd, 'node_modules', 'streetjs', 'package.json'))) ??
        ...
        try {
            ...
            for (let i = 0; i < 6; i++) {
                try {
                    ...
                    if (pkg.name === 'streetjs' && pkg.version)
                        return pkg.version;
                }
                catch {
                    /* keep walking up */
