...
let ContactController = class ContactController {
    async submit(ctx) {
        ...
        if (!b?.['name'] || !b?.['email'] || !b?.['subject'] || !b?.['message']) {
            ctx.json({ error: { code: 'INVALID_BODY', message: 'name, email, subject, message required' } }, 400);
            return;
        }
        const email = String(b['email']);
        if (!email.includes('@')) {
            ctx.json({ error: { code: 'INVALID_EMAIL', message: 'Valid email address required' } }, 400);
            return;
        }
        const pool = getPool();
        const id = randomUUID();
        await pool.query(`INSERT INTO contact_messages (id, name, email, phone, company, subject, message)
       VALUES ($1,$2,$3,$4,$5,$6,$7)`, [id, b['name'], email, b['phone'] ?? null, b['company'] ?? null, b['subject'], b['message']]);
    ...
    async updateStatus(ctx) {
        ...
        if (!['NEW', 'IN_PROGRESS', 'RESOLVED', 'SPAM'].includes(status)) {
            ctx.json({ error: { code: 'INVALID_STATUS', message: 'Invalid status value' } }, 400);
            return;
        }
        const pool = getPool();
        await pool.query(`UPDATE contact_messages SET status = $1, updated_at = NOW() WHERE id = $2`, [status, ctx.params['id']]);
