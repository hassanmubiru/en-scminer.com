...
let ReviewController = class ReviewController {
    async list(ctx) {
        ...
        const [dataRes, countRes, summaryRes] = await Promise.all([
            pool.query(`SELECT r.id, r.rating, r.title, r.body, r.is_verified_purchase, r.created_at,
                u.first_name, u.last_name
         FROM reviews r
         JOIN users u ON u.id = r.user_id
         WHERE r.product_id = $1 AND r.status = 'APPROVED'
         ORDER BY r.created_at DESC LIMIT $2 OFFSET $3`, [ctx.params['productId'], limit, offset]),
            pool.query(`SELECT COUNT(*) AS total FROM reviews WHERE product_id = $1 AND status = 'APPROVED'`, [ctx.params['productId']]),
            pool.query(`SELECT AVG(rating) AS avg_rating, COUNT(*) AS total_count,
         ...
         FROM reviews WHERE product_id = $1 AND status = 'APPROVED'`, [ctx.params['productId']]),
        ]);
    ...
    async create(ctx) {
        ...
        if (!user) {
            ctx.json({ error: { code: 'UNAUTHORIZED', message: 'Login required to submit a review' } }, 401);
            return;
        }
        const body = ctx.body;
        const rating = Number(body?.['rating']);
        if (!body || rating < 1 || rating > 5) {
            ctx.json({ error: { code: 'INVALID_RATING', message: 'Rating must be 1–5' } }, 400);
            return;
        }
        const pool = getPool();
        // Check for existing pending/approved review
        const existing = await pool.query(`SELECT id FROM reviews WHERE product_id = $1 AND user_id = $2 AND status != 'REJECTED' LIMIT 1`, [ctx.params['productId'], user.id]);
        if (existing.rows.length > 0) {
            ctx.json({ error: { code: 'ALREADY_REVIEWED', message: 'You have already reviewed this product' } }, 409);
            return;
        }
        const id = randomUUID();
        await pool.query(`INSERT INTO reviews (id, product_id, user_id, rating, title, body)
       VALUES ($1,$2,$3,$4,$5,$6)`, [id, ctx.params['productId'], user.id, rating,
    ...
    async moderate(ctx) {
        ...
        if (!['APPROVED', 'REJECTED'].includes(status)) {
            ctx.json({ error: { code: 'INVALID_STATUS', message: 'status must be APPROVED or REJECTED' } }, 400);
            return;
        }
        const pool = getPool();
        await pool.query(`UPDATE reviews SET status = $1, updated_at = NOW() WHERE id = $2`, [status, ctx.params['reviewId']]);
        // Update product rating aggregate
        await pool.query(`UPDATE products p SET
         ...
         updated_at   = NOW()
       WHERE id = (SELECT product_id FROM reviews WHERE id = $1)`, [ctx.params['reviewId']]);
...
__decorate([
    ...
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], ReviewController.prototype, "moderate", null);
