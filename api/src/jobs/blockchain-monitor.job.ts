...
export async function runBlockchainMonitorJob(): Promise<void> {
  ...
  try {
    ...
    const res = await pool.query(
      `SELECT p.id, p.wallet_id, p.crypto_amount, p.expires_at,
...
async function checkPayment(payment: Record<string, unknown>): Promise<void> {
...
async function checkBitcoin(paymentId: string, address: string): Promise<void> {
  const btcRpc = process.env['BTC_RPC_URL'];
  if (!btcRpc) return; // Not configured
  ...
  const txids = result['txids'] as string[];
  ...
  for (const txHash of txids) {
    ...
    const txData = await txResp.json() as Record<string, unknown>;
    const tx = txData['result'] as Record<string,unknown>;
    if (!tx) continue;

    const amount = String(Math.abs(Number(result['amount'])));
    const confirmations = Number(tx['confirmations'] ?? 0);

    await svc.processDetectedTransaction(
      paymentId, txHash, 'BITCOIN', '', address,
      amount, 'BTC', confirmations,
    );
  }
}

async function checkEthereum(paymentId: string, address: string, contractAddress: string | null): Promise<void> {
  ...
  const blockData = await blockResp.json() as Record<string, unknown>;
  ...
  if (contractAddress) {
    ...
    const logsData = await logsResp.json() as Record<string, unknown>;
    const logs = (logsData['result'] as Array<Record<string,unknown>>) ?? [];
    ...
    for (const log of logs) {
      ...
      await svc.processDetectedTransaction(
        paymentId, txHash, 'ETHEREUM', '', address,
        amount, 'USDT', confirmations,
      );
    }
  }
}

async function checkTron(paymentId: string, address: string): Promise<void> {
  const tronUrl = process.env['TRON_API_URL'];
  if (!tronUrl) return;

  const resp = await fetch(
    `${tronUrl}/v1/accounts/${address}/transactions/trc20?limit=20&contract_address=TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t`,
    { signal: AbortSignal.timeout(10_000) },
  );
  if (!resp.ok) return;
  const data = await resp.json() as Record<string, unknown>;
  ...
  for (const tx of txs) {
    ...
    const value  = String((tx['value'] as string | undefined) ?? '0');
    ...
    await svc.processDetectedTransaction(
      paymentId, txHash, 'TRON', '', address,
      amount, 'USDT', 20, // TRON only needs ~20 blocks
    );
  }
}
