import { Controller, Get, Patch, ApiOperation, Injectable } from 'streetjs';
...
export class AdminController {
  ...
  private isAdmin(ctx: StreetContext): boolean {
  ...
  async dashboard(ctx: StreetContext): Promise<void> {
    if (!this.isAdmin(ctx)) { ctx.json({ error: { code: 'FORBIDDEN', message: 'Admin required' } }, 403); return; }
    const pool = getPool();

    const [orders, revenue, customers, products, pendingPayments, pendingReviews, newMessages] =
      await Promise.all([
    ...
    ctx.json({
      orders: orders.rows,
      totalRevenueCents: Number((revenue.rows[0] as Record<string,unknown>)['total'] ?? 0),
      totalRevenue: Number((revenue.rows[0] as Record<string,unknown>)['total'] ?? 0) / 100,
      totalCustomers: Number((customers.rows[0] as Record<string,unknown>)['total'] ?? 0),
      totalProducts:  Number((products.rows[0] as Record<string,unknown>)['total'] ?? 0),
      pendingPayments: Number((pendingPayments.rows[0] as Record<string,unknown>)['total'] ?? 0),
      pendingReviews:  Number((pendingReviews.rows[0] as Record<string,unknown>)['total'] ?? 0),
      newMessages:     Number((newMessages.rows[0] as Record<string,unknown>)['total'] ?? 0),
    });
  }

  @Get('/customers')
  @ApiOperation({ summary: '[Admin] List customers', tags: ['admin'] })
  async customers(ctx: StreetContext): Promise<void> {
    if (!this.isAdmin(ctx)) { ctx.json({ error: { code: 'FORBIDDEN', message: 'Admin required' } }, 403); return; }
    const pool = getPool();
    ...
    const [data, count] = await Promise.all([
      pool.query(
        `SELECT id, email, first_name, last_name, phone, company, status, created_at
    ...
    ctx.json({
      items: data.rows,
      total: Number((count.rows[0] as Record<string,unknown>)['total'] ?? 0),
      page, limit,
    });
  }
  ...
  async customerStatus(ctx: StreetContext): Promise<void> {
    const user = getUser(ctx);
    if (!this.isAdmin(ctx)) { ctx.json({ error: { code: 'FORBIDDEN', message: 'Admin required' } }, 403); return; }
    const b = ctx.body as Record<string, unknown> | null;
    const status = String(b?.['status'] ?? '');
    if (!['active','suspended'].includes(status)) {
      ctx.json({ error: { code: 'INVALID_STATUS', message: 'status must be active or suspended' } }, 400); return;
    }
    const pool = getPool();
  ...
  async pendingPayments(ctx: StreetContext): Promise<void> {
    if (!this.isAdmin(ctx)) { ctx.json({ error: { code: 'FORBIDDEN', message: 'Admin required' } }, 403); return; }
    const pool = getPool();
    const res = await pool.query(
      `SELECT p.id, p.order_id, p.method, p.status, p.amount_cents, p.crypto_amount,
    ...
    ctx.json({ items: res.rows.map(r => ({ ...(r as Record<string,unknown>), amount: Number((r as Record<string,unknown>)['amount_cents'])/100 })) });
  }
  ...
  async manualConfirm(ctx: StreetContext): Promise<void> {
    const user = getUser(ctx);
    if (!user?.roles.some(r => r === 'super_admin')) {
      ctx.json({ error: { code: 'FORBIDDEN', message: 'super_admin required for manual payment confirmation' } }, 403); return;
    }
    const pool = getPool();
    const before = await pool.query(`SELECT status, order_id FROM payments WHERE id = $1`, [ctx.params['id']]);
    if (!before.rows[0]) { ctx.json({ error: { code: 'NOT_FOUND', message: 'Payment not found' } }, 404); return; }

    const prev = before.rows[0] as Record<string, unknown>;
    await pool.query(
      `UPDATE payments SET status = 'CONFIRMED', updated_at = NOW() WHERE id = $1`, [ctx.params['id']],
    );
    await pool.query(
      `UPDATE orders SET status = 'PAID', updated_at = NOW() WHERE id = $1`, [prev['order_id']],
    );
    await this.audit.log({
      actorId: user.id,
  ...
  async auditLog(ctx: StreetContext): Promise<void> {
    if (!this.isAdmin(ctx)) { ctx.json({ error: { code: 'FORBIDDEN', message: 'Admin required' } }, 403); return; }
    const q = ctx.query as Record<string, string | undefined>;
  ...
  async updatePaymentStatus(ctx: StreetContext): Promise<void> {
    const user = getUser(ctx);
    if (!this.isAdmin(ctx)) { ctx.json({ error: { code: 'FORBIDDEN', message: 'Admin required' } }, 403); return; }
    ...
    const before = await pool.query(
      `SELECT status, order_id FROM payments WHERE id = $1`, [ctx.params['id']],
    );
    if (!before.rows[0]) { ctx.json({ error: { code: 'NOT_FOUND', message: 'Payment not found' } }, 404); return; }
  ...
  async getSettings(ctx: StreetContext): Promise<void> {
    if (!this.isAdmin(ctx)) { ctx.json({ error: { code: 'FORBIDDEN', message: 'Admin required' } }, 403); return; }
    const pool = getPool();
  ...
  async updateSetting(ctx: StreetContext): Promise<void> {
    const user = getUser(ctx);
    if (!user?.roles.some(r => ['admin','super_admin'].includes(r))) {
      ctx.json({ error: { code: 'FORBIDDEN', message: 'Admin required' } }, 403); return;
    }
    const b = ctx.body as Record<string, unknown> | null;
    if (!b || !('value' in b)) {
      ctx.json({ error: { code: 'INVALID_BODY', message: 'value required' } }, 400); return;
    }
    const pool = getPool();
    ...
    await pool.query(
      ...
      [ctx.params['key'], JSON.stringify(b['value'])],
    );
    await this.audit.log({
      actorId: user.id,
