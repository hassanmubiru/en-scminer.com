...
export class CompareController {
  ...
  async get(ctx: StreetContext): Promise<void> {
    ...
    if (!user && !sessionId) { ctx.json({ items: [] }); return; }
    ...
    const res = await pool.query(
      `SELECT p.id, p.slug, p.name, p.price_cents, p.hashrate, p.hashrate_unit,
  ...
  async add(ctx: StreetContext): Promise<void> {
    ...
    if (Number((count.rows[0] as Record<string,unknown>)['c']) >= MAX_COMPARE) {
      ctx.json({ error: { code: 'COMPARE_LIMIT', message: `Maximum ${MAX_COMPARE} products for comparison` } }, 400);
      return;
    }

    await pool.query(
      `INSERT INTO compare_items (id, user_id, session_id, product_id) VALUES ($1,$2,$3,$4)
       ON CONFLICT DO NOTHING`,
      [randomUUID(), user?.id ?? null, user ? null : sessionId, body['productId']],
    );
